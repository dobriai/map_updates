...
var bManualTesting = 'm' in argv;
var bAutoTesting = 'a' in argv;
...
app.get('/fullupdate', function (req, res) {
  res.send(JSON.stringify(userData));
})
if (bManualTesting) {
  app.get('/faker/:uid', function (req, res) {
    res.render('faker',
    {
      GOOGLE_API_KEY: GOOGLE_API_KEY,
      LAT_LNG_0: JSON.stringify(lalos[0]),
...
function broadcastUpdate(userUpdate) {
  userData[ userUpdate['uid'] ] = _.omit(userUpdate, ['uid']);
...
wss.on('connection', function (ws)
{
  ...
  if (bManualTesting) {
    ws.on('message', function (msg) {
      ...
      if (Object.keys(jIn).length != msgKeys.length) {
        console.log(`*** Input has more or fewer values: ${Object.keys(jIn).length}!`)
        return;
      }
      var userUpdate = jIn;
      broadcastUpdate(userUpdate);
    });
  }
});
